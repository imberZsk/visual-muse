...
export type PlatformId = "wechat" | "zhihu" | "toutiao" | "juejin" | "csdn" | "medium";
...
function parseSimpleFrontmatter(metadataText: string): Record<string, unknown> {
  ...
  for (const rawLine of metadataText.split(/\r?\n/)) {
    ...
    if (currentListKey && trimmedLine.startsWith("- ")) {
      ...
      const currentList: unknown[] = Array.isArray(currentRawList) ? currentRawList : [];
 ...
 */
export function buildWechatDraftPayload(article: ParsedArticle): WechatDraftPayload {
 ...
 */
export function validatePublishTarget(platform: PublishPlatform, article: ParsedArticle): PublishValidation {
  // 错误列表，保存阻止发布的必填项问题。
  const errors: string[] = [];
  // 警告列表，保存不阻止发布的体验优化建议。
  ...
  if (platform.id === "wechat" && !article.metadata.cover && extractMarkdownImages(article.body).length === 0) {
 ...
 */
export function simulatePublish(platform: PublishPlatform, article: ParsedArticle): PublishResult {
...
function normalizeMetadata(data: Record<string, unknown>): ArticleMetadata {
  ...
  return {
    ...
    image_list: Array.isArray(rawImageList) ? rawImageList.map(String) : undefined,
...
function isImageArticle(article: ParsedArticle, imageList: string[]): boolean {
  return article.metadata.type === "image" || imageList.length > 0 && Boolean(article.metadata.image_list);
}
