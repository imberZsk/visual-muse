...
export const publishingPlatforms: PublishPlatform[] = [
  {
    ...
    name: '微信公众号',
    capability:
      '生成图文内容并填入公众号官方编辑器，保留本地或远程 Server 扩展能力。',
    supportsSimulation: true,
  },
  {
    id: 'xiaohongshu',
    name: '小红书',
    capability: '把标题和正文填入小红书长文编辑器，复用独立持久登录会话。',
    supportsSimulation: true,
  },
  {
    id: 'zhihu',
  ...
  },
  {
    ...
    name: '掘金',
    capability:
      '保留 Markdown 结构并填入掘金编辑器，继续在官方页面设置分类和标签。',
    supportsSimulation: true,
  },
  {
    id: 'csdn',
...
/**
 * 解析文章可用标题；`article` 是已解析文章，优先使用 frontmatter，再使用正文首个一级标题。
 */
export function resolveArticleTitle(
  article: ParsedArticle
): string | undefined {
  // 元数据标题，保存用户在 frontmatter 中显式设置的发布标题。
  const metadataTitle = article.metadata.title?.trim()
  if (metadataTitle) {
    return metadataTitle
  }

  // 一级标题 token，使用 Markdown 解析器避免把代码块里的 `#` 误判为文章标题。
  const headingToken = marked
    .lexer(article.body)
    .find((token) => token.type === 'heading' && token.depth === 1)
  if (!headingToken || headingToken.type !== 'heading') {
    return undefined
  }

  // 一级标题文本，保存去除首尾空白后的发布标题兜底值。
  const headingTitle = headingToken.text.trim()
...
export function validatePublishTarget(
  ...
  if (!resolveArticleTitle(article)) {
    errors.push('缺少文章标题')
  }
...
function resolveTitle(article: ParsedArticle): string {
  return resolveArticleTitle(article) || '未命名文章'
}
