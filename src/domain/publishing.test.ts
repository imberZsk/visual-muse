...
describe('文章发布领域模型', () => {
  ...
  test('正文一级标题可作为发布标题', () => {
    // 没有 frontmatter 的标准 Markdown，用来验证一级标题兜底发布标题。
    const article = parseArticleMarkdown('# 没有 frontmatter 标题')

    expect(resolveArticleTitle(article)).toBe('没有 frontmatter 标题')
    expect(
      validatePublishTarget(publishingPlatforms[0], article)
    ).toMatchObject({
      ok: true,
      errors: [],
    })
  ...
  test('没有 frontmatter 和一级标题时预检提示缺失标题', () => {
    // 纯正文文章解析结果，用来验证发布前错误提示仍然有效。
    const article = parseArticleMarkdown('只有普通正文')
